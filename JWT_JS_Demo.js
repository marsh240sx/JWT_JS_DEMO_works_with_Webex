...
class JWT_JS_Demo
{
    ...
    CreateJWT(Header, Payload, Secret)
    {
        ...
        let base64Secret = CryptoJS.enc.Base64.parse(Secret);


        let signature = CryptoJS.HmacSHA256(base64Header + "." + base64Payload, base64Secret);
        let base64Sign = CryptoJS.enc.Base64.stringify(signature).toString(CryptoJS.enc.Base64).replace(/\+/g,'-').replace(/\//g,'_').replace(/\=+$/m,'');
